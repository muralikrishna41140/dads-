...
function Notes() {
  ...
  return (
    <AppShell title={t("notes.title")} subtitle={t("notes.subtitle")}>
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-3xl border border-border bg-card p-5 shadow-soft">
          <div className="text-xs text-muted-foreground">{t("notes.total")}</div>
        ...
        <div className="rounded-3xl border border-warning/30 bg-warning/10 p-5 shadow-soft">
          <div className="text-xs font-medium text-warning">{t("notes.pending")}</div>
        ...
        <div className="rounded-3xl border border-success/30 bg-success/10 p-5 shadow-soft">
          <div className="text-xs font-medium text-success">{t("notes.completed")}</div>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        ...
        <AddNote
          onAdd={(n) => {
            addNote(n);
            toast.success(t("notes.addToast"));
          }}
        />
      </div>
        ...
        {filtered.map((n, i) => (
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-1.5">
                  <span
                    className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${PRIO_STYLE[n.priority]}`}
                  >
                    {n.priority}
                  </span>
                ...
                <h3
                  className={`mt-2 font-display text-base font-bold ${n.status === "Completed" ? "line-through" : ""}`}
                >
                  {n.title}
                </h3>
                {n.content && (
              ...
              <button
                ...
                className={`shrink-0 rounded-lg p-1.5 ${n.pinned ? "text-primary" : "text-muted-foreground"} hover:bg-accent`}
              >
            ...
            <div className="mt-4 flex items-center gap-2">
              <Button
                size="sm"
                variant={n.status === "Completed" ? "outline" : "default"}
                className={`rounded-full text-xs ${n.status === "Pending" ? "bg-gradient-brand" : ""}`}
                onClick={() => toggleNote(n.id)}
              >
                <Check className="mr-1 h-3.5 w-3.5" />{" "}
                {n.status === "Completed" ? t("notes.reopen") : t("notes.markDone")}
              </Button>
              <button
                onClick={() => {
                  deleteNote(n.id);
                  toast(t("notes.deleteToast"));
                }}
...
function AddNote({
  ...
  onAdd: (n: Parameters<ReturnType<typeof useStore>["addNote"]>[0]) => void;
}) {
  const { t } = useI18n();
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="rounded-full bg-gradient-brand shadow-elegant">
          <Plus className="mr-1 h-4 w-4" /> {t("notes.addNote")}
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{t("notes.addDialogTitle")}</DialogTitle>
        </DialogHeader>
